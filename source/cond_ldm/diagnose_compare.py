...
from config import CondLDMDiagConfig, UNetConfig, VAEConfig
...
def _parse_id_from_path(path: str) -> str:
    # e.g. ./data/raw_png/b800/<ID>_b800/xxx.png
...
def _pick_indices_from_distinct_ids(
    ...
    chosen = [(ids[i], id_to_first_index[ids[i]]) for i in perm]
    return chosen
...
def main() -> None:
    ...
    summary_lines = ["id,index,mse_teacher,mse_cond"]
    ...
    for i, (id_, dataset_index) in enumerate(chosen):
        ...
        pred_teacher = teacher(zt, t_batch)
        pred_cond = cond(latents=zt, timesteps=t_batch, dwi_image=x_dwi)

        mse_teacher = float(F.mse_loss(pred_teacher, noise).item())
        mse_cond = float(F.mse_loss(pred_cond, noise).item())
        mse_teacher_all.append(mse_teacher)
        mse_cond_all.append(mse_cond)
        summary_lines.append(
            f"{id_},{int(dataset_index)},{mse_teacher:.8f},{mse_cond:.8f}"
        )
        ...
        out_path = os.path.join(
            diag_cfg.out_dir,
            f"id_{id_}_idx{int(dataset_index):06d}_t{t_fixed}_steps{len(reverse_ts)}.png",
        )
        save_image(grid, out_path, nrow=4)

    mean_teacher = sum(mse_teacher_all) / max(len(mse_teacher_all), 1)
    mean_cond = sum(mse_cond_all) / max(len(mse_cond_all), 1)
    summary_lines.append(f"MEAN,_,{mean_teacher:.8f},{mean_cond:.8f}")

    with open(
        os.path.join(diag_cfg.out_dir, f"t{t_fixed}_mse_summary.csv"),
        "w",
        encoding="utf-8",
    ) as f:
        f.write("\n".join(summary_lines) + "\n")
    ...
    print(f"t={t_fixed} mean MSE teacher={mean_teacher:.6f} cond={mean_cond:.6f}")
