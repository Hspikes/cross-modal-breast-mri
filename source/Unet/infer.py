...
from config import UnetInferConfig as InferConfig
...
def load_unet(
    ...
    unet_cfg = (
        ckpt.get("config", default_config) if isinstance(ckpt, dict) else default_config
    )
...
def infer_one(
    *,
    ...
    denoised_imgs = []
    for t_int in ts:
        t = torch.full((x.size(0),), int(t_int), device=x.device, dtype=torch.long)
...
def main():
    ...
    unet_default_cfg = UNetConfig(latent_channels=vae_cfg.latent_channels)
    unet = load_unet(unet_default_cfg, cfg.unet_ckpt_path, device)

    for i, dataset_index in enumerate(chosen):
        item = eval_dataset[int(dataset_index)]
        x = item["gt"].to(device).unsqueeze(0)  # [1,1,H,W]
